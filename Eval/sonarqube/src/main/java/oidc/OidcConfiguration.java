...
public class OidcConfiguration {
  private static final String CATEGORY = "CATEGORY_SECURITY" ;//CATEGORY_SECURITY;
  ...
  public OidcConfiguration(Properties config) {
    this.config = config;
  }

  public boolean isEnabled() {
    return Boolean.parseBoolean(config.getProperty(ENABLED));//getBoolean(ENABLED).orElse(false) && issuerUri() != null && clientId() != null;
  }

  @CheckForNull
  public String issuerUri() {
    return config.getProperty(ISSUER_URI, null); //.orElse(null);
  }

  @CheckForNull
  public String clientId() {
    return config.getProperty(CLIENT_ID, null); //.orElse(null);
  }

  public String clientSecret() {
    return config.getProperty(CLIENT_SECRET, null); //.orElse(null);
  }

  public String scopes() {
    return config.getProperty(SCOPES, null);//.orElse("openid");
  }

  public boolean allowUsersToSignUp() {
    return Boolean.parseBoolean(config.getProperty(ALLOW_USERS_TO_SIGN_UP));//.getBoolean(ALLOW_USERS_TO_SIGN_UP).orElse(false);
  }
/*
