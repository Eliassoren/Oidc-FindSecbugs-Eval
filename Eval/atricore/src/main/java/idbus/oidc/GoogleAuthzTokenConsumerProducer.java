...
//import com.google.api.services.oauth2.Oauth2; FIXME: altered - api interface is deprecated or changed
//import com.google.api.services.oauth2.model.Userinfoplus;
import idbus.oidc.camel.AbstractCamelEndpoint;
...
// import org.atricore.idbus.capabilities.openidconnect.main.common.OpenIDConnectException; Alter to SecurityException instead to save code
...
public class GoogleAuthzTokenConsumerProducer extends AuthzTokenConsumerProducer {
    ...
    private Properties mediationReplacementToCompile;
    ...
    protected void doProcessAuthzTokenResponse(/*CamelMediationExchange exchange,*/ AuthorizationCodeResponseUrl authnResp ) throws Exception {
       ...
       // MediationState mediationState = in.getMessage().getState();
        OpenIDConnectProxyMediator mediator = new OpenIDConnectProxyMediator();// (OpenIDConnectProxyMediator) channel.getIdentityMediator();
     ...
     //   flow.get
        ...
        if (authnResp.getError() != null) {
            ...
            throw new SecurityException("OpenId Connect error: " + authnResp.getError() + " " +  authnResp.getErrorDescription());
        } else if (code == null) {
            logger.error("Missing authorization code ");
            throw new SecurityException("Illegal response, no authorization code received ");
        }

        // Validate relay state

        String expectedRelayState = (String) mediationReplacementToCompile.getProperty("relayState");//mediationState.getLocalVariable("urn:OPENID-CONNECT:1.0:relayState");
        ...
        if (!expectedRelayState.equals(relayState)) {
            ...
            throw new SecurityException("Illegal response, received OpenID Connect state is not valid");
        }
        ...
        String accessTokenConsumerLocation = "some/url";
        ...
        IdTokenResponse idTokenResponse = (IdTokenResponse) mediator.sendMessage(request, accessTokenConsumerLocation);/* accessTokenConsumerLocation, channel);*/
        ...
        // TODO Below: won't compile. Here userinfo request happens. No token validation is found
        // get user info // FIXME: altered - deprecated API interfacing
