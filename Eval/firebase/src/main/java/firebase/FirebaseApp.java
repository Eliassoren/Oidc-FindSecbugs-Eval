...
import static firebase.FirebaseOptions.APPLICATION_DEFAULT_CREDENTIALS;
 ...
 * applications don't need to directly interact with firebase.FirebaseApp.
 *
 * <p>Firebase APIs use the default firebase.FirebaseApp by default, unless a different one is explicitly
...
public class FirebaseApp {
  ...
  /** A map of (name, firebase.FirebaseApp) instances. */
  ...
  /**
   * Per application lock for synchronizing all firebase.internal firebase.FirebaseApp state changes.
   */
  ...
  public static     FirebaseApp getInstance(@NonNull String name) {
    synchronized (appsLock) {
      ...
      String errorMessage =
          String.format(
              "firebase.FirebaseApp with name %s doesn't exist. %s", name, availableAppNamesMessage);
  ...
  static  FirebaseApp initializeApp(FirebaseOptions options, String name,
    ...
    synchronized (appsLock) {
      checkState(
          ...
          "firebase.FirebaseApp name " + normalizedName + " already exists!");
  ...
  private void checkNotDeleted() {
    ...
    // guava versions.
    checkState(!deleted.get(), "firebase.FirebaseApp '%s' was deleted", new Object[]{name});
  }
  ...
  /**
   * Utility class for scheduling proactive token refresh events. Each firebase.FirebaseApp should have
