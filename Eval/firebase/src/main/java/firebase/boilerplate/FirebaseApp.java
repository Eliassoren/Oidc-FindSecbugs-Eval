...
import static firebase.boilerplate.FirebaseOptions.APPLICATION_DEFAULT_CREDENTIALS;

/**
 * The entry point of Firebase SDKs. It holds common configuration and state for Firebase APIs. Most
 * applications don't need to directly interact with firebase.boilerplate.FirebaseApp.
 *
 * <p>Firebase APIs use the default firebase.boilerplate.FirebaseApp by default, unless a different one is explicitly
...
public class FirebaseApp {
  ...
  /** A map of (name, firebase.boilerplate.FirebaseApp) instances. */
  ...
  /**
   * Per application lock for synchronizing all firebase.internal firebase.boilerplate.FirebaseApp state changes.
   */
  ...
  public static     FirebaseApp getInstance(@NonNull String name) {
    synchronized (appsLock) {
      ...
      String errorMessage =
          String.format(
              "firebase.boilerplate.FirebaseApp with name %s doesn't exist. %s", name, availableAppNamesMessage);
  ...
  static  FirebaseApp initializeApp(FirebaseOptions options, String name,
    ...
    synchronized (appsLock) {
      checkState(
          ...
          "firebase.boilerplate.FirebaseApp name " + normalizedName + " already exists!");
  ...
  private void checkNotDeleted() {
    ...
    // guava versions.
    checkState(!deleted.get(), "firebase.boilerplate.FirebaseApp '%s' was deleted", new Object[]{name});
  }
  ...
  /**
   * Utility class for scheduling proactive token refresh events. Each firebase.boilerplate.FirebaseApp should have
