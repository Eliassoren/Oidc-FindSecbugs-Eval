...
import firebase.boilerplate.ExportedUserRecord;
import firebase.boilerplate.FirebaseToken;
import firebase.boilerplate.FirebaseTokenVerifier;
...
// FIXME: changes - added public identifier to clean up so all doesn't have to be in same package
...
public final class FirebaseTokenVerifierImpl implements FirebaseTokenVerifier {
   ...
   * @throws ExportedUserRecord.FirebaseAuthException If the input token string does not meet any of the conditions
   *     listed above.
   */
  @Override
  ...
  private IdToken parse(String token) throws ExportedUserRecord.FirebaseAuthException {
    try {
      ...
      String detailedError = String.format(
          ...
          docUrl,
          articledShortName);
      throw new ExportedUserRecord.FirebaseAuthException(ERROR_INVALID_CREDENTIAL, detailedError, e);
    }
  }

  private void checkContents(final IdToken token) throws ExportedUserRecord.FirebaseAuthException {
    ...
    if (errorMessage != null) {
      ...
      throw new ExportedUserRecord.FirebaseAuthException(ERROR_INVALID_CREDENTIAL, detailedError);
    }
  }

  private void checkSignature(IdToken token) throws ExportedUserRecord.FirebaseAuthException {
    try {
      if (!isSignatureValid(token)) {
        throw new ExportedUserRecord.FirebaseAuthException(ERROR_INVALID_CREDENTIAL,
            String.format(
  ...
  /**
   * Verifies the cryptographic signature on the firebase.boilerplate.FirebaseToken. Can block on a web request to fetch
  ...
  private String getErrorForTokenWithoutKid(Header header, Payload payload) {
  ...
  private boolean isCustomToken(Payload payload) {
    return FIREBASE_AUDIENCE.equals(payload.getAudience());
  }

  private boolean isLegacyCustomToken(Header header, Payload payload) {
    return "HS256".equals(header.getAlgorithm())
        && new BigDecimal(0).equals(payload.get("v"))
        && containsLegacyUidField(payload);
  }

  private boolean containsLegacyUidField(Payload payload) {
