...
/**
 * firebase.FirebaseUserManager provides methods for interacting with the Google Identity Toolkit via its
...
class FirebaseUserManager {
  ...
  /**
   * Creates a new firebase.FirebaseUserManager instance.
   *
  ...
  FirebaseUserManager(@NonNull FirebaseApp app, @Nullable HttpRequestFactory requestFactory) {
    checkNotNull(app, "firebase.FirebaseApp must not be null");
    String projectId = ImplFirebaseTrampolines.getProjectId(app);
    checkArgument(!Strings.isNullOrEmpty(projectId),
        "Project ID is required to access the auth service. Use a service account credential or "
            + "set the project ID explicitly via firebase.FirebaseOptions. Alternatively you can also "
    ...
    UserImportRequest(List<ImportUserRecord> users, UserImportOptions options,
      ...
      if (hasPassword) {
        checkArgument(options != null && options.getHash() != null,
            "firebase.UserImportHash option is required when at least one user has a password. Provide "
                + "a firebase.UserImportHash via firebase.UserImportOptions.withHash().");
